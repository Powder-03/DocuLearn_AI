...
async def create_learning_plan(request: CreatePlanRequest, db: Session = Depends(get_db)) -> CreatePlanResponse:
    """
...
async def get_session(
    ...
    """
    try:
        session = await run_in_threadpool(session_service.get_session, db=db, session_id=session_id, user_id=user_id)
...
async def delete_session(
    ...
    """
    try:
        result = await run_in_threadpool(session_service.delete_session, db=db, session_id=session_id, user_id=user_id)
        return result
...
async def get_user_statistics(user_id: str, db: Session = Depends(get_db)):
    ...
    """
    try:
        stats = await run_in_threadpool(session_service.get_session_statistics, db=db, user_id=user_id)
